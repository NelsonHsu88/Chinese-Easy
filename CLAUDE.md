...
- `src/app/(tabs)/` — five `Tabs.Screen` slots (Dashboard, Lessons, Review, Dictionary, Settings), but only **Dashboard** (`index`), **Dictionary** (`dictionary-tab`, which re-exports the real screen) and **Settings** actually switch tab content. The other two intercept `tabPress` with `e.preventDefault()` in `(tabs)/_layout.tsx`:
  ...
  - **Lessons** opens a `TabPickerSheet` letting the user choose between `/lessons`, `/new-words` and `/books`.
  - `(tabs)/review-tab.tsx` is a redirect-only fallback that exists purely so `Tabs.Screen` has a route to point at. **Do not** name it (or `dictionary-tab.tsx`) `review.tsx` / `dictionary.tsx` — that would collide with the real standalone routes at `src/app/review.tsx` / `src/app/dictionary.tsx` (same URL path) and expo-router's navigator loops indefinitely instead of reaching the real screen.
- Standalone routes pushed on top of the tabs (not nested in them): `/review`, `/review-session`, `/due-words`, `/onboarding`, `/profile`, `/dictionary`, `/new-words`, `/my-words`, `/my-town`, `/lessons`, `/lesson/[lessonId]`, `/books`, `/story/[storyId]`, `/challenges`, `/radicals`.
...
`src/data/hskFrequency.ts` merges a small hand-curated word list with a large bulk-imported dictionary (`src/data/importedWords.json`) into the full word bank. Both carry example sentences, but not uniformly: all curated entries have one (with pinyin), while roughly 8,000 of the 20,000 imported entries do (without pinyin). **Example sentences come from the bundled corpus and are never generated** — code that needs one filters for words that have it rather than inventing text (see `newWordsPool` in `selectors.ts`). `src/data/mockDeck.ts` seeds a demo SRS deck; `src/data/placementTest.ts` is the fixed onboarding placement-test word list.
...
- `src/data/stories.ts` (`STORIES`) is a hand-authored reading library — 46 stories across HSK 1-6, weighted towards the upper levels (folk tales, chengyu stories, festival legends and classical myths). Later stories set `pinyin: ''`: the reader renders a per-word reading from the word bank under each word, so a second romanised copy of the whole paragraph would be redundant and would have to be hand-checked syllable by syllable. Only `translation` is required on every page. `src/screens/StoryReader.tsx` (route `/story/[storyId]`) segments each page's Chinese text with `segmentText` (`textSegmentation.ts`) so known/unknown words are tappable; tapping an unrecognized word adds it to the deck via `addWordFromBook`, which also flags it in `newlyAddedWordIds` (surfaced as a badge dot on the Dictionary tab icon until `clearNewWordFlags` runs).
